...
from ..models import Weapon, Rarity, WeaponType, HandsRequired
...
class WeaponRepository(BaseRepository):
    ...
    def create(self, weapon: Weapon) -> Weapon:
        ...
        weapon.id = self._execute_insert(
            """INSERT INTO weapons
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (weapon.world_id, weapon.campaign_id, weapon.name, weapon.description,
             weapon.type.value, weapon.hands_required.value, weapon.attack,
             weapon.rarity.value, weapon.value)
        )
        return weapon
    ...
    def get_by_world(self, world_id: int, campaign_id: Optional[int] = None) -> list[Weapon]:
        ...
        if campaign_id is not None:
            ...
            rows = self.db.conn.execute(
                "SELECT * FROM weapons WHERE world_id = ? AND campaign_id IS NULL",
                (world_id,)
            ).fetchall()
    ...
    def update(self, weapon: Weapon) -> None:
        ...
        self.db.conn.execute(
            ...
            (weapon.world_id, weapon.campaign_id, weapon.name, weapon.description,
             weapon.type.value, weapon.hands_required.value, weapon.attack,
             weapon.rarity.value, weapon.value, weapon.id)
        )
        self.db.conn.commit()
