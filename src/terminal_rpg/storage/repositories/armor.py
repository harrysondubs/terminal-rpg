...
from ..models import Armor, ArmorType, Rarity
...
class ArmorRepository(BaseRepository):
    ...
    def get_by_id(self, armor_id: int) -> Armor | None:
        """Fetch single armor by ID"""
        row = self._fetch_by_id("armor", armor_id)
        return self._row_to_armor(row) if row else None

    def get_by_world(self, world_id: int, campaign_id: int | None = None) -> list[Armor]:
        ...
        if campaign_id is not None:
            ...
            rows = self.db.conn.execute(
                "SELECT * FROM armor WHERE world_id = ? AND campaign_id IS NULL", (world_id,)
            ).fetchall()
