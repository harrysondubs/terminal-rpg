...
class CampaignLogRepository(BaseRepository):
    ...
    def create(self, log: CampaignLog) -> CampaignLog:
        ...
        log.id = self._execute_insert(
            """INSERT INTO campaign_logs
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (log.campaign_id, log.world_id, log.location_id, log.battle_id,
             log.type.value, log.content)
        )
        log.created_at = self._fetch_timestamp('campaign_logs', log.id)
        return log

    def get_by_id(self, log_id: int) -> Optional[CampaignLog]:
    ...
    def get_by_campaign(self, campaign_id: int, limit: Optional[int] = None) -> list[CampaignLog]:
    ...
    def get_by_battle(self, battle_id: int) -> list[CampaignLog]:
        ...
        rows = self.db.conn.execute(
            "SELECT * FROM campaign_logs WHERE battle_id = ? ORDER BY created_at DESC",
            (battle_id,)
        ).fetchall()
        return [self._row_to_log(row) for row in rows]
